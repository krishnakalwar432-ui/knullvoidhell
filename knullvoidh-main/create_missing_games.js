...
const generateGameComponent = (template) => {
  const getColorClass = (color) => {
    if (color === '#ffffff') return 'gray';
...
// Create all missing game files
let successCount = 0;
let errorCount = 0;

gameTemplates.forEach(template => {
  try {
    const componentContent = generateGameComponent(template);
    const filePath = path.join(__dirname, 'client', 'pages', 'games', `${template.name}.tsx`);
    
    fs.writeFileSync(filePath, componentContent, 'utf8');
    console.log(`✓ Created ${template.name}.tsx`);
    successCount++;
  } catch (error) {
    console.error(`✗ Error creating ${template.name}.tsx:`, error.message);
    errorCount++;
  }
});
